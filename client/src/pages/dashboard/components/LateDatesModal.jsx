...
import {CreditCardSelect} from "../../../components/creditCards/CreditCardSelect.jsx";
...
export const LateDatesModal = ({expense, handleClose, onPaymentSuccess}) => {
    ...
    const paySelectedMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            invalidateLateDateCaches(qc, expense.id);
            invalidatePaymentsForExpense(qc, expense.id);
        ...
        onError: (error) => {
            if (getStatus(error) === 401) {
                ...
                showApiErrorToast(error, 'Failed to pay selected dates.');
            }
        },
    });
    ...
    const skipDateMutation = useMutation({
        ...
        onError: (error) => {
            if (getStatus(error) === 401) {
                ...
                showApiErrorToast(error, 'Failed to skip date.');
            }
        },
        onSettled: () => {
    ...
    return (
        <>
            <Modal
                ...
                className="app-modal late-dates-modal"
                showSave={false}
                    ...
                    !showPaymentDateInput ? (
                        <>
                            ...
                            <button
                                ...
                                disabled={selectedDates.length === 0 || paySelectedMutation.isPending}
                            >
